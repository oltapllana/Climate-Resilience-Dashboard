...
function Heatmap({ yearly, t }) {
  ...
  return <ChartFrame custom description={t("heavySnowDesc")} t={t} rows={matrixRows(yearly)} columns={matrixColumns} indicator="heavy-snowfall-matrix"><div style={{ position: "relative", overflowX: "auto" }} onMouseLeave={() => setTip(null)}>
  ...
  return <section className="card landslide-indicator">
    <div className="indicator-grid">
      <div className="indicator-panel">
        ...
        <div className="event-list"><strong>{t("heavySnowEvents")}</strong>{result.events.map((event) => <div key={event.date}>{event.date}: {t.number(event.precipitation, { minimumFractionDigits: 1, maximumFractionDigits: 1 })} mm, {t.number(event.meanTemperature, { minimumFractionDigits: 1, maximumFractionDigits: 1 })}°C</div>)}</div>
      </div>
      <div className="indicator-panel">
        <div className="indicator-heading"><h2>{t("annualTotal")}</h2><p>{t("heavySnowAnnualDesc")}</p></div>
        <ChartFrame t={t} rows={result.yearly} columns={[{"key":"year","label":"Year"},{"key":"annualTotal","label":"Heavy snowfall days"},{"key":"isPartial","label":"Partial year"}]} indicator="heavy-snowfall-indicator-1" width="100%" height={360}><BarChart data={result.yearly} layout="vertical" margin={{ top: 16, right: 42, left: 18, bottom: 42 }}><CartesianGrid stroke="#dce5ea" horizontal={false} /><XAxis tickFormatter={(value) => t.number(value)} type="number" allowDecimals={false} domain={[0, "dataMax + 1"]} label={{ value: t("heavySnowAxis"), position: "insideBottom", offset: -24, style: { textAnchor: "middle", fill: "#475569", fontSize: 12, fontWeight: 600 } }} /><YAxis type="category" dataKey="year" width={54} tickFormatter={(value) => `${value}${result.yearly.find((row) => row.year === value)?.isPartial ? "*" : ""}`} /><Tooltip content={<AnnualTooltip t={t} />} /><Bar dataKey="annualTotal" name={t("heavySnowAxis")} minPointSize={3} radius={[0, 3, 3, 0]}>{result.yearly.map((row) => <Cell key={row.year} fill={row.isPartial ? "#8999a2" : "#397f99"} />)}<LabelList dataKey="annualTotal" content={<ValueLabel />} /></Bar></BarChart></ChartFrame>
